...
$services = @(
    ...
    @{ name='frontend'; cwd=Join-Path $scriptRoot '..'; command='npm run start:local'; ports=@(4201); log='frontend.log' },
...
function Start-ServiceWindow($svc) {
    ...
    # For services that need env injection (backend, dotnet), create a small
    # temporary wrapper PS1 file that sets the env vars and runs the command.
    if ($svc.name -in @('backend','schools-api-dotnet')) {
...
function Start-All {
    ...
    foreach ($svc in $services) { Start-ServiceWindow $svc; Start-Sleep -Milliseconds 500 }
