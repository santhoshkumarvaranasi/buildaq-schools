import { a as j, b as z, c as ie, d as se, e as re, f as ne, g as ae, h as oe, i as ce, j as he, k, l as C, m as de, n as _e, o as me, p as le, q as ue, r as Ie, s as fe, t as pe, u as ve } from "@nf-internal/chunk-KBSX3DDG";
...
var Ee = 200, h = class {
    ...
    _setupKeyHandler(e) {
        this._letterKeyStream.pipe(ge(t => this._pressedLetters.push(t)), be(e), Ae(() => this._pressedLetters.length > 0), xe(() => this._pressedLetters.join("").toLocaleUpperCase())).subscribe(t => {
...
var c = class {
    ...
    onKeydown(e) {
        let t = e.keyCode, s = ["altKey", "ctrlKey", "metaKey", "shiftKey"].every(n => !e[n] || this._allowedModifierKeys.indexOf(n) > -1);
        switch (t) {
            case 9:
                this.tabOut.next();
                return;
...
var d = class {
    ...
    _initializeFocus() {
        ...
        let t = this._items[e];
        t.makeFocusable ? (this._activeItem?.unfocus(), this._activeItemIndex = e, this._activeItem = t, this._typeahead?.setCurrentSelectedItemIndex(e), t.makeFocusable()) : this.focusItem(e), this._hasInitialFocused = !0;
    }
    ...
    focusItem(e, t = {}) {
        t.emitChangeEvent ??= !0;
        let i = typeof e == "number" ? e : this._items.findIndex(a => this._trackByFn(a) === this._trackByFn(e));
        if (i < 0 || i >= this._items.length)
            return;
        let s = this._items[i];
        if (this._activeItem !== null && this._trackByFn(s) === this._trackByFn(this._activeItem))
            return;
        let n = this._activeItem;
        this._activeItem = s ?? null, this._activeItemIndex = i, this._typeahead?.setCurrentSelectedItemIndex(i), this._activeItem?.focus(), n?.unfocus(), t.emitChangeEvent && this.change.next(this._activeItem), this._shouldActivationFollowFocus && this._activateCurrentItem();
    }
    _updateActiveItemIndex(e) {
        let t = this._activeItem;
        if (!t)
            return;
        let i = e.findIndex(s => this._trackByFn(s) === this._trackByFn(t));
        i > -1 && i !== this._activeItemIndex && (this._activeItemIndex = i, this._typeahead?.setCurrentSelectedItemIndex(i));
    }
    ...
    _findNextAvailableItemIndex(e) {
        for (let t = e + 1; t < this._items.length; t++)
            if (!this._skipPredicateFn(this._items[t]))
                return t;
        return e;
    }
    _findPreviousAvailableItemIndex(e) {
        for (let t = e - 1; t >= 0; t--)
            if (!this._skipPredicateFn(this._items[t]))
                return t;
        return e;
    ...
    _expandAllItemsAtCurrentItemLevel() {
        if (!this._activeItem)
            return;
        let e = this._activeItem.getParent(), t;
        e ? t = f(e.getChildren()) : t = Oe(this._items.filter(i => i.getParent() === null)), t.pipe(S(1)).subscribe(i => {
            for (let s of i)
                s.expand();
        });
    }
...
var gt = "cdk-describedby-message-container", K = "cdk-describedby-message", _ = "cdk-describedby-host", v = 0, bt = (() => {
    class r {
        _platform = o(A);
        _document = o(L);
        ...
        describe(t, i, s) {
            if (!this._canBeDescribed(t, i))
                return;
            let n = p(i, s);
            typeof i != "string" ? (P(i, this._id), this._messageRegistry.set(n, { messageElement: i, referenceCount: 0 })) : this._messageRegistry.has(n) || this._createMessageElement(i, s), this._isElementDescribedByMessage(t, n) || this._addMessageReference(t, n);
        }
        removeDescription(t, i, s) {
            if (!i || !this._isElementNode(t))
                return;
            let n = p(i, s);
            if (this._isElementDescribedByMessage(t, n) && this._removeMessageReference(t, n), typeof i == "string") {
        ...
        ngOnDestroy() {
            let t = this._document.querySelectorAll(`[${_}="${this._id}"]`);
            for (let i = 0; i < t.length; i++)
                this._removeCdkDescribedByReferenceIds(t[i]), t[i].removeAttribute(_);
        ...
        _createMessagesContainer() {
            if (this._messagesContainer)
                return;
            let t = "cdk-describedby-message-container", i = this._document.querySelectorAll(`.${t}[platform="server"]`);
            for (let n = 0; n < i.length; n++)
                i[n].remove();
            let s = this._document.createElement("div");
            s.style.visibility = "hidden", s.classList.add(t), s.classList.add("cdk-visually-hidden"), this._platform.isBrowser || s.setAttribute("platform", "server"), this._document.body.appendChild(s), this._messagesContainer = s;
        }
        ...
        _canBeDescribed(t, i) {
            ...
            if (i && typeof i == "object")
                return !0;
            let s = i == null ? "" : `${i}`.trim(), n = t.getAttribute("aria-label");
            return s ? !n || n.trim() !== s : !1;
        }
...
var y = class {
    ...
    focusItem() { }
}, At = { provide: O, useFactory: () => () => new y }, g = class extends C {
    _focusTrapManager;
    _inertStrategy;
...
}, ze = new Le("FOCUS_TRAP_INERT_STRATEGY"), Ye = (() => {
    class r {
        _focusTrapStack = [];
