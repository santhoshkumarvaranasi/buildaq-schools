...
var _ = class extends m {
    ...
    viewCacheSize = 20;
    _viewCache = [];
    applyChanges(t, i, s, n, e) {
        t.forEachOperation((h, f, E) => {
            let a, l;
            if (h.previousIndex == null) {
                let w = () => s(h, f, E);
                a = this._insertView(w, E, i, n(h)), l = a ? o.INSERTED : o.REPLACED;
            }
            else
                E == null ? (this._detachAndCacheView(f, i), l = o.REMOVED) : (a = this._moveView(f, E, i, n(h)), l = o.MOVED);
            e && e({ context: a?.context, operation: l, record: h });
        });
    }
    detach() {
        for (let t of this._viewCache)
            t.destroy();
        this._viewCache = [];
    }
    _insertView(t, i, s, n) {
        let e = this._insertViewFromCache(i, s);
        if (e) {
            e.context.$implicit = n;
            return;
        }
        let h = t();
        return s.createEmbeddedView(h.templateRef, h.context, h.index);
    }
