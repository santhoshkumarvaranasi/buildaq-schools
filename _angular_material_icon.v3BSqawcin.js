...
var I;
function H() { if (I === void 0 && (I = null, typeof window < "u")) {
    let i = window;
...
var l = class {
    ...
    addSvgIconLiteralInNamespace(t, e, n, o) { let r = this._sanitizer.sanitize(m.HTML, n); if (!r)
        throw w(n); let s = p(r); return this._addSvgIconConfig(t, e, new l("", s, o)); }
    ...
    addSvgIconSetLiteralInNamespace(t, e, n) { let o = this._sanitizer.sanitize(m.HTML, e); if (!o)
        throw w(e); let r = p(o); return this._addSvgIconSetConfig(t, new l("", r, n)); }
    ...
    getSvgIconFromUrl(t) { let e = this._sanitizer.sanitize(m.RESOURCE_URL, t); if (!e)
        throw F(t); let n = this._cachedIconsByUrl.get(e); return n ? g(S(n)) : this._loadSvgIconFromConfig(new l(t, null)).pipe(C(o => this._cachedIconsByUrl.set(e, o)), _(o => S(o))); }
    getNamedSvgIcon(t, e = "") { let n = y(e, t), o = this._svgIconConfigs.get(n); if (o)
        return this._getSvgFromConfig(o); if (o = this._getIconConfigFromResolvers(e, t), o)
        return this._svgIconConfigs.set(n, o), this._getSvgFromConfig(o); let r = this._iconSetConfigs.get(e); return r ? this._getSvgFromIconSetConfigs(t, r) : x(E(n)); }
    ...
    _getSvgFromIconSetConfigs(t, e) { let n = this._extractIconWithNameFromAnySet(t, e); if (n)
        return g(n); let o = e.filter(r => !r.svgText).map(r => this._loadSvgIconSetFromConfig(r).pipe(P(s => { let d = `Loading icon set URL: ${this._sanitizer.sanitize(m.RESOURCE_URL, r.url)} failed: ${s.message}`; return this._errorHandler.handleError(new Error(d)), g(null); }))); return U(o).pipe(_(() => { let r = this._extractIconWithNameFromAnySet(t, e); if (!r)
        throw E(t); return r; })); }
    ...
    _extractSvgIconFromSet(t, e, n) { let o = t.querySelector(`[id="${e}"]`); if (!o)
        ...
        return this._setSvgAttributes(this._toSvgElement(r), n); let s = this._svgElementFromString(p("<svg></svg>")); return s.appendChild(r), this._setSvgAttributes(s, n); }
    ...
    _fetchIcon(t) { let { url: e, options: n } = t, o = n?.withCredentials ?? !1; if (!this._httpClient)
        throw R(); if (e == null)
        throw Error(`Cannot fetch icon from URL "${e}".`); let r = this._sanitizer.sanitize(m.RESOURCE_URL, e); if (!r)
        throw F(e); let s = this._inProgressUrlFetches.get(r); if (s)
        return s; let u = this._httpClient.get(r, { responseType: "text", withCredentials: o }).pipe(_(d => p(d)), D(() => this._inProgressUrlFetches.delete(r)), z()); return this._inProgressUrlFetches.set(r, u), u; }
...
var J = ["*"], K = new b("MAT_ICON_DEFAULT_OPTIONS"), G = new b("mat-icon-location", { providedIn: "root", factory: () => { let i = f(k), c = i ? i.location : null; return { getPathname: () => c ? c.pathname + c.search : "" }; } }), A = ["clip-path", "color-profile", "src", "cursor", "fill", "filter", "marker", "marker-start", "marker-mid", "marker-end", "mask", "stroke"], Q = A.map(i => `[${i}]`).join(", "), X = /^url\(['"]?#(.*?)['"]?\)$/, vt = (() => {
    class i {
        ...
        _cacheChildrenWithExternalReferences(t) { let e = t.querySelectorAll(Q), n = this._elementsWithExternalReferences = this._elementsWithExternalReferences || new Map; for (let o = 0; o < e.length; o++)
            A.forEach(r => { let s = e[o], u = s.getAttribute(r), d = u ? u.match(X) : null; if (d) {
                let v = n.get(s);
